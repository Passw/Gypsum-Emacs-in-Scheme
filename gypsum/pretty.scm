...
(define (run-tests)
  ...
  (define test-mode 'unit-test-mode)
    ...
    (list
     ...
     (test-case
      ...
      (print
       ...
       "\n\nHello\n\nHello"))
     (test-case
      (new-line-buffer-stats 14 14 0 14 0 0)
      "one two three\n"
      (print (join-by #\space "one" "two" "three") (line-break)))
     (test-case
      "one\ntwo\nthree\n"
      (new-line-buffer-stats 5 3 0 14 0 0)
      (print (join-lines "one" "two" "three") (line-break)))
     (test-case
      "(one\n    two\n    three)\n"
      (new-line-buffer-stats 5 3 0 24 0 4)
      (print (bracketed 4 #\( #\) (join-lines "one" "two" "three"))))
     ))
     ...
     (let-values
       ...
       (cond
        ...
        ((eq? test-mode 'print-mode)
         ...
         (display " ... ")
         (cond
          ((and (equal? expected-string result-string)
                (or (not result-stats)
                    (equal? expected-stats result-stats)))
           (display "OK\n"))
          (else
           (display "failed\n   result-string: ") (write result-string) (newline)
           (display " expected-string: ") (write expected-string) (newline)
      ...
      (lambda (test-case)
        (let-values
            (((expected-string) (test-case-expected-string test-case))
             ...
             ((result-str result-stats)
              (run-test-print-proc test-runner (test-case-printer test-case)))
             )
          (cond
           ((eq? test-mode 'unit-test-mode)
            (test-equal expected-string result-str)
            (test-equal expected-stats result-stats))
           ((eq? test-mode 'print-mode)
            (unless
                (and (equal? expected-string result-str)
                     (or
                      (not result-stats)
                      (equal? expected-stats result-stats)))
              (display "result-string: ") (write result-str) (newline)

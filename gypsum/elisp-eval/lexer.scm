...
;; --- We are using the "define-library" and a separate ".sld" file to
;; construct the module now, as this is more portable.
;;
;; (define-module (gypsum elisp-eval lexer)
...
;;--------------------------------------------------------------------------------------------------
;; This module was originally written in Guile. I am rewriting it in
;; pure R7RS standard Scheme.
...
(define (and=> value proc) (if value (proc value) #f))

(define make-symbol string->symbol)
   ;; ^ not sure if these are homomorphic, rename the standard API so
   ;;   it can be easily restored later if necessary.,

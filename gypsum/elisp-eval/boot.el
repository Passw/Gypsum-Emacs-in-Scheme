...
(defun symbol-function (symbol)
  (let ((f (%funcall (@ (gypsum elisp-eval runtime) symbol-function)
                     symbol)))
    (if (%funcall (@ (gypsum elisp-eval falias) falias?) f)
        (%funcall (@ (gypsum elisp-eval falias) falias-object) f)
      f)))
...
(defun fset (symbol definition)
  (funcall (@ (gypsum elisp-eval runtime) set-symbol-function!)
           symbol
           (if (functionp definition)
               definition
             (funcall (@ (gypsum elisp-eval falias) make-falias)
...
(fset 'symbol-value (@ (gypsum elisp-eval runtime) symbol-value))
(fset 'set (@ (gypsum elisp-eval runtime) set-symbol-value!))
(fset 'makunbound (@ (gypsum elisp-eval runtime) makunbound!))
(fset 'fmakunbound (@ (gypsum elisp-eval runtime) fmakunbound!))
(fset 'boundp (@ (gypsum elisp-eval runtime) symbol-bound?))
(fset 'fboundp (@ (gypsum elisp-eval runtime) symbol-fbound?))
(fset 'intern (@ (guile) string->symbol))

(defun defvaralias (new-alias base-variable &optional docstring)
  (let ((fluid (funcall (@ (gypsum elisp-eval runtime) symbol-fluid)
                        base-variable)))
    (funcall (@ (gypsum elisp-eval runtime) set-symbol-fluid!)
             new-alias
             fluid)
    base-variable))
